...
function removeAddresses(array) {
  let mass = array.find(function(item){
...
function deleteUser(array, id) {
  const hum = array.filter(function(item){
    if (item.id === id){
      return delete item
    } else {
      return item
...
function getUsersGeo(array, id) {
  return array.find(function(item){
    if (item.id === id){
      return console.log(item.address.geo)
    }
  })
}
...
function getUsersCompany(array, id) {
  return array.find(function(item){
    if(item.id === id){
      return console.log(item.company)
    }
  })
}
...
function changePhone(array, id, phone) {
  return array.reduce(function (acc, item){
    if(item.id === id){
      return item.phone = acc
    }
  }, acc = phone)
}

changePhone(users, 1, `9-999-999-9999 x55555`)
